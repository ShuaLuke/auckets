...
- **Security controls stay ON across every service we use — that's the default, not a per-case decision.** RLS on every Postgres table, signature verification on every webhook, env validation, auth on every route, least-privilege keys. If a service offers a security control (RLS, 2FA, signed requests, IP allowlists, audit logs, etc.), it should be enabled. Turning one *off* requires an ADR explaining why; turning one *on* never does. When adding a new table, integration, or vendor, the question is "are all its controls on?" — not "do we need them yet?"
- **RLS is ON (deny-all) on all public tables.** Every table has RLS enabled with no policies, so the anon/PostgREST path is fully denied; the app reaches Postgres only through Drizzle server-side with the direct connection. New tables **must** enable RLS; the `rls_enabled_no_policy` INFO advisors are the intended state — do not "fix" them. Adding any policy requires an ADR. See [docs/ARCHITECTURE.md §Database](docs/ARCHITECTURE.md) and [SECURITY.md rule 26](docs/SECURITY.md#database-and-data-handling).
- **Supabase publishable/anon key is a private credential.** No `NEXT_PUBLIC_SUPABASE_*` vars, no `@supabase/supabase-js` in the client. App talks to Postgres only through Drizzle server-side. See [SECURITY.md rule 26](docs/SECURITY.md#database-and-data-handling).
