...
*Status: requirements agreed 2026-09-07. All six slices built (PRs #136 → #141, stacked) except the admin Simulation tab, which waits on Julia's design outline — see `sim/README.md` for usage. Temporal, seat-preference and upgrade numbers model questions Cope has not yet answered; they inform the decision, they don't record one.*
...
| **S6** ✅ (tab pending) | `lookahead[:rows]` as an opt-in fit policy (defer at most one fitting offer per row when the window strands less; never drop); `protect-units` (`unitPolicy: "protect"`) for tables/boxes; seat preferences beyond tier **scored, not enforced** (aisle / centre / front satisfied-by-chance rates); upgrade buyouts after binding in the timeline (Q29, ADR-0014 uplift to artist). On Cope's pool: lookahead 1,139 seated, +$1,550, 1 passed over. Supper-club tables: protect costs ~17/114 seats. Lincoln seat prefs by chance: aisle 57%, centre 24%, front 6%. **Admin Simulation tab not built** — Julia's design outline isn't in the repo; the pure core is ready to sit behind it | Cope's Phase 6; NEW-14; playbook prefs; Q29 |
