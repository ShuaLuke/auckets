...
**Status:** Accepted & **implemented** under the working assumption: ≤6-day offer window + auth-based hold (still pending Cope's final confirmation; if his research lands on windows >6 days we revert to the SetupIntent body below)
**Date:** 2026-05-25 (original) · 2026-05-27 (working-assumption note) · 2026-05-31 (shipped #58–#82 against the assumption)
...
**2026-05-25 note:** Cope flagged "still outstanding need to do research on this" in v2. The SetupIntent path stands as the recommendation; if Cope's research lands on "keep offer windows ≤ 6 days and use a normal Stripe pre-auth," we revisit. ~~Don't ship the offer-submission flow (Week 4) until this is settled.~~ *Superseded by the 2026-05-27 working assumption below — we proceeded with the ≤6-day pre-auth path.*
...
**This assumption is still not confirmed by Cope.** It was recorded so subsequent slices could build against a concrete model rather than block — and they have: the real `POST /api/offers` PaymentIntent path (#58–#61), binding capture/release (#62), scheduled binding (#78), ticket issuance (#81), and card-failure recovery (#79/#80) all shipped against it. Once Cope confirms (or rejects) the ≤6-day constraint, this note becomes the canonical decision OR we revert to the SetupIntent body above and revisit the slices built against the assumption.

**Status as of 2026-05-31:** the assumption is fully implemented in production. The dev stub (`ALLOW_DEV_OFFER_STUB`) remains only as a no-Stripe fallback, refused on production.
